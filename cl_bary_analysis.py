...
from baryon_analysis_mainscript import data_key, data, data_same, z_same, base_index, colors
...
L = np.arange(10, lmax+1, 50, dtype=np.float64)
...
for datakey in data_key:
    ...
    for j, Xmax in enumerate(X_max):
        ...
        if j == 0:
            ...
            ax2.semilogx(L, (cl_bary_zmax-cl_base_ALLzmax[j])/cl_base_ALLzmax[j] - (cl_bary_z6-cl_base_ALLzmax[j])/cl_base_ALLzmax[j], color=colors[j], label=r'$z_{max}$' + '= {0}'.format(zmax))
    ...
    ax1.grid(True)
    #fig1.show()
    fig1.savefig('{0}zmax_analysis/{1}_ratio_vs_zmax.pdf'.format(savefolder, datakey))
    
    ax2.legend()
    ax2.set_ylabel(r'$\frac{(C_\ell^{\kappa\kappa, bary} - C_\ell^{\kappa\kappa, DMONLY})}{C_\ell^{\kappa\kappa, DMONLY}}$($z_{max}$) - $\frac{(C_\ell^{\kappa\kappa, bary} - C_\ell^{\kappa\kappa, DMONLY})}{C_\ell^{\kappa\kappa, DMONLY}}$($z_{max}$ = 6)')
    ax2.set_xlabel(r'$\ell$')
    ax2.grid(True)
    
    # (C_\ell^{\kappa\kappa, bary}$ - $C_\ell^{\kappa\kappa, DMONLY})
    # $\frac{}{}$
